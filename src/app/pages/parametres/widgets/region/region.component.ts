...
export class RegionComponent implements OnInit {
  ...
  displayedColumns: string[] = ['position', 'pays', 'code', 'libelle', 'action'];
  ...
  exportToExcel() {
    this.data.exportToExcel(this.List, 'Region');
  }
  ...
  openDialog2(id: any) {
    const dialogRef = this.dialog.open(RegionUpdateComponent,
      { data:{ids: id}, disableClose: true });
  ...
  openDialogCreation() {
    const dialogRef = this.dialog.open(RegionCreationComponent,
      // { data:{ids: id}, disableClose: true }
      );
